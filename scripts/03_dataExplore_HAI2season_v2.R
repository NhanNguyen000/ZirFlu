...
# per strains
pList <- list()
for (type in unique(HAI_ab$strains)) {
  HAI_ab_temp <- HAI_ab %>% filter(strains == type)
  bxp <- HAI_ab_temp %>%
    ggboxplot(x = "season", y = "ab_value", fill = "disease",
              bxp.errorbar = TRUE, palette = "npg",
              outlier.shape = 1, outlier.size = 4, outlier.color = "grey")
  
  stat.test <- HAI_ab_temp %>% group_by(season) %>%
    t_test(ab_value ~ disease)%>% add_xy_position(x = "season", dodge = 0.8)
  ...
  pList[[type]] <- bxp + 
    stat_pvalue_manual(
      stat.test, label = "p", tip.length = 0.01, bracket.nudge.y = 1)  + 
    scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
    ylab(paste0("log2_", type))
}

cowplot::plot_grid(pList$H1N1_abFC, pList$H3N2_abFC,
                   pList$Bvictoria_abFC, pList$Byamagata_abFC,
                   nrow = 2)
