...
selected_proteins1 <- unique(c(proSig_ab_2019, proSig_ab_2020))
selected_proteins2 <- unique(c(proSig_onlyAb_2019, proSig_onlyAb_2020))
selected_proteins <- selected_proteins1[-which(selected_proteins1 %in% selected_proteins2)]
...
picked_metabolite <- ZirFlu$metabolite_annot %>% 
  filter(Formula %in% highlight_metabolites$Formula)
picked_metabolite <- ZirFlu$metabolite_annot %>% 
  filter(Formula %in% c("C20H30O5", ""))
selected_metabolites <- unique(picked_metabolite$ionIdx)
...
plotDat_long_2seasons %>% 
  ...
  #geom_text(aes(label = ifelse(significance_pvalue == TRUE, "*", ""))) +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red") + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + theme_bw()
