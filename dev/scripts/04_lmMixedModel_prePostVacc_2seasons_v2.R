...
b <- get.proteinAnnot(ZirFlu$protein_annot, a)
...
# other code
topTable(lmRes_prePost_protein$`2019`$T1vsT3, coef = 2)
volcanoplot(lmRes_prePost_protein$`2019`$T1vsT3, coef = "timeT3", highlight = 10, 
            names = rownames(lmRes_prePost_protein$`2019`$T1vsT3$coefficients))
abline(h = -log10(0.05))

plotMD(lmRes_prePost_protein$`2019`$T1vsT3, column = 2)
qqt(lmRes_prePost_protein$`2019`$T1vsT3$t[,2], 
    df = lmRes_prePost_protein$`2019`$T1vsT3$df.residual + lmRes_prePost_protein$`2019`$T1vsT3$df.prior)
abline(0,1)
...
sigPval_metabolites <- unique(c(rownames(resPrePost_metabolite$`2019`$T1vsT2$pval),
         rownames(resPrePost_metabolite$`2019`$T1vsT3$pval),
         rownames(resPrePost_metabolite$`2020`$T1vsT2$pval),
         rownames(resPrePost_metabolite$`2020`$T1vsT3$pval)))

picked_metabolites <- ZirFlu$metabolite_annot %>% 
  filter(ionIdx %in% sigPval_metabolites) %>% select(ionIdx, Formula) %>% distinct()

load("temp/20221222_keggID_allMetabolites.RData")
selected_metabolites_T2andT3 <- keggID_allMetabolites %>% filter(ionIdx %in% sigPval_metabolites)

write.table(selected_metabolites_T2andT3, file = "temp/20221223_selected_metabolites_T2andT3.txt", sep = "\t",
